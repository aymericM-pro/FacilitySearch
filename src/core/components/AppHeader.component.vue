...
const userPhoto = computed(() => profileStore.profile?.photoUrl || null);
        ...
        <template #right>
            <template v-if="isAuthenticated">
                ...
                <FsPopover align="right">
                    <template #trigger>
                        <div
                            class="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center transition hover:scale-105 cursor-pointer"
                        >
                            <img
                                v-if="userPhoto"
                                :src="userPhoto"
                            ...
                            <div
                                v-else
                                class="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-semibold text-sm"
                            >
                                {{ userInitials }}
                            </div>
                        </div>
